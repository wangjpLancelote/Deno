          ...
          >
            ...
            >
              <template slot-scope="scope">
                <div class="flex" style="padding-left:10px;justify-content:center;">
                  <span v-if="showUid" style="margin-left:30px;">
                    {{ scope.row.title + '(' + scope.row.uid + ')' }}
                  </span>
                  <span v-else style="margin-left:30px;">{{ scope.row.title }}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template slot-scope="scope">
                <div class="flex" style="width:100%;height:40px;align-items:center;justify-content:center;">
      ...
      <div v-if="cache.length" class="overHide margin_top tagsCall">
        ...
        >
          <span><i class="el-icon-paperclip" style="font-size:14px;margin-right:3px;" /> {{
            item.title ? item.title : item.uid ? item.uid : item
          }}</span>
        </el-tag>
      </div>
      <slot />
  ...
  methods: {
    ...
    serviceFunc (body) {
      ...
      if (this.method === 'post') {
        ...
        return service[this.method](this.url + '?' + `offset=${this.offset * this.limit}&limit=${this.limit}&order=-created`, body);
      } else if (this.method === 'get') {
        return service[this.method](this.url + '?' + `offset=${this.offset * this.limit}&limit=${this.limit}&order=-created`, { params: { ...body, ...this.params }});
      } else {
        return service[this.method](this.url + '?' + `offset=${this.offset * this.limit}&limit=${this.limit}&order=-created`, body);
      }
    },
    ...
    requestServiceDef () {
      ...
      const service = this.axios.create({
        baseURL: 'http://newcms-dev.benewtech.cn/api/',
        timeout: 20000
      });
    ...
    search () {
      ...
      if (this.searchCondition) {
        ...
        if (target.some(v => Number(v))) {
          this.queryParams.userId = target.map(Number);
        } else {
    ...
    exclude (idx, uid) {
      ...
      let indexData = this.data.find(v => v.uid === uid);
      if (indexData) this.data.find(v => v.uid === uid).lock = false;
      this.cache.splice(idx, 1);
    },
    ensure() {
      this.$emit('getBook', this.cache);
      this.closeDialog();
    }
  }
};
