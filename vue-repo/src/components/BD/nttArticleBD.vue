    ...
    <div class="box">
      <div class="flex box_outer">
        <el-input
          ...
          placeholder="输入文章标题或UID, 按Enter键搜索"
          style="width:98%;"
          clearable
  ...
  methods: {
    ...
    filter (source, target) {
      ...
      for (let item of source) {
        if (item && item.uid) item = item.uid;
    ...
    async getContextData () {
      ...
      let indexUrl = this.url + `/filterId/undefined?offset=${this.offset * this.limit}&limit=${this.limit}&action=load&order=-created`;
      let rt = await this.serviceFunc(indexUrl, { dbLoadQuery: body });
      if (rt) {
    ...
    search () {
      ...
      if (this.searchCondition) {
        ...
        if (target.some(v => Number(v))) {
          this.queryParams.uid = {'$in': target.map(Number)};
        } else {
          this.queryParams.title = this.searchCondition;
        }
      }
      this.offset = 0;
    ...
    exclude (idx, uid) {
      ...
      this.$emit('getArticle', this.checked);
      this.closeDialog();
    }
  }
};
